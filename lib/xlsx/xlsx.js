...
function xlsx(file) { 
   ...
   if (typeof file === 'string') { // Load
		...
		if (s = zip.file('xl/sharedStrings.xml')) { // Process sharedStrings
			var si = s.asText().split(/<si.*?>/g);
			i = si.length;
			while(--i) { 
				var w = unescapeXML(si[i].substring(0,si[i].indexOf('</si>'))).split(/<t.*?>/g),
				wl = w.length, wa = [];
				for (var wi = 0; wi < wl; wi++) {
					wa[wi] = unescapeXML(w[wi].substring(0, w[wi].indexOf('</t>')))
				}
				sharedStrings[i - 1] = wa.join('');
